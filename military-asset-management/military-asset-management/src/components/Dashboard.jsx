  ...
  const fetchData = async () => {
    try {
      const [assetsRes, transfersRes, assignmentsRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/assets`, {
          headers,
        }),
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/transfers`, {
          headers,
        }),
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/assignments`, {
          headers,
        }),
      ]);
            ...
            <tbody>
              {filteredAssets.map((asset) => (
                <tr key={asset.id}>
                  ...
                  <td>
                    <span
                      className={`badge ${asset.status === "Available" ? "available" : "assigned"}`}
                    >
                      {asset.status}
                    </span>
                  </td>
      ...
      {selectedAsset && (
        ...
        >
          ...
          >
            ...
            >
              <h2 style={{ margin: 0 }}>
                Movement History: {selectedAsset.assetName}
              </h2>
              <button
              ...
              <ul style={{ paddingLeft: "1.5rem" }}>
                {getAssetDetails(selectedAsset.id).assetTransfers.map((t) => (
                  <li key={t.id} style={{ marginBottom: "0.5rem" }}>
                    Moved from <strong>{t.fromBase}</strong> to{" "}
                    <strong>{t.toBase}</strong> on {t.date}
                  </li>
                ))}
              </ul>
            ) : (
